  ...
  return (
    <div className="no-print pointer-events-none sticky bottom-0 z-20 px-3 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-2 sm:px-4">
      <div className="pointer-events-auto mx-auto max-w-4xl overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--bg)] shadow-[var(--shadow)]">
          ...
          <div className="border-b border-[var(--line)]">
            <div className="flex items-center justify-between gap-2 px-4 py-2 text-sm text-[var(--muted)]">
              ...
              <span className="flex items-center gap-1">
                ...
                <button className="grid h-8 w-8 place-items-center rounded-lg hover:bg-[var(--hover)]" onClick={() => setOpen(false)} aria-label="Collapse chat">
                  <Icon name="x" size={16} />
                </button>
              </span>
            </div>
                  ...
                  >
                    ...
                    {m.filed && m.filed.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {m.filed.map((f, i) => (
                          <span key={i} className="chip fn-pop" style={{ animationDelay: `${i * 70}ms` }}>{f}</span>
                        ))}
                      </div>
                    )}
                    {m.splitTxId && (
        ...
        {(recording || voice !== "off") && (
          <div className="flex items-center gap-2 border-b border-[var(--line)] px-4 py-2.5 text-sm text-[var(--muted)]">
